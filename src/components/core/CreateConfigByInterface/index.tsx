...
const UserInterfaceModal = forwardRef<UserInterfaceModalHandle | undefined>(
  (_, ref) => {
    ...
    function handleChangeAlgo() {
      if (form.values.algorithmName) {
        console.log("form.values.algorithmName", form.values.algorithmName);
        const spec =
          algorithmSpec[
            form.values.algorithmName as keyof typeof algorithmSpec
          ];
        console.log("spec", spec);

        form.setFieldValue("surrogate.surrogate", spec.surrogate);
        form.setFieldValue("acquisition.acquisition", spec.acquisition);
        form.setFieldValue("solver.solver", spec.solver);
        form.setFieldValue("selection.selection", spec.selection);
      }
    }
                ...
                {form.values.type === "Problem" && (
                  <>
                    ...
                    <NumberInput
                      ...
                      withAsterisk
                      min={2}
                      {...form.getInputProps("n_init_sample")}
                    ></NumberInput>
                  </>
                )}
                ...
                {form.values.type === "Optimization" && (
                  <>
                    <Select
                      ...
                      withAsterisk
                      data={Object.keys(algorithmSpec)}
                      onSelect={handleChangeAlgo}
                    ...
                    {form.values.advancedSettingType === "Surrogate" && (
                      <>
                        <Select
                          label="Name"
                          withAsterisk
                          data={surrogateNames.map((v) => ({
                            label: v,
                            value: acronym(v),
                          }))}
                          {...form.getInputProps("surrogate.surrogate")}
                        ></Select>
                        <NumberInput
                          label="nu"
                          {...form.getInputProps("surrogate.nu")}
                        ></NumberInput>
                      </>
                    )}
                    ...
                    {form.values.advancedSettingType === "Acquisition" && (
                      <>
                        <Select
                          ...
                          {...form.getInputProps("acquisition.acquisition")}
                        ></Select>
                        <NumberInput
                          label="n_spectral_pts"
                          {...form.getInputProps("acquisition.n_spectral_pts")}
                        ></NumberInput>
                        <Checkbox
                          label="mean_sample"
                          {...form.getInputProps("acquisition.meanSample", {
                            type: "checkbox",
                          })}
                        ></Checkbox>
                      </>
                    ...
                    {form.values.advancedSettingType === "Solver" && (
                      <>
                        <Select
                          ...
                          withAsterisk
                          data={solverNames}
                          {...form.getInputProps("solver.solver")}
                        ></Select>
                        <NumberInput
                          label="n_gen"
                          {...form.getInputProps("solver.n_gen")}
                        ></NumberInput>
                        <NumberInput
                          label="pop_size"
                          {...form.getInputProps("solver.pop_size")}
                        ></NumberInput>
                      </>
                    )}
                    {form.values.advancedSettingType === "Selection" && (
                      <>
                        <Select
                          ...
                          withAsterisk
                          data={selectionNames}
                          {...form.getInputProps("selection.selection")}
                        ></Select>
                      </>
                    )}
                  </>
